    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index(UsuarioModel usuario)
        {
            using (var context = _http.CreateClient())
            {
                ...
                ViewBag.Mensaje = "Usuario o contrase�a incorrecta.";
                return View();
            }
        }
        ...
        public async Task<IActionResult> RecoverPassword(string correo, [FromServices] ProyectoTransportesMana.Services.IEmailService emailService)
        {
            ...
            {
                var msg = "La API devolvi� error al solicitar el token.";
                try
                {
            ...
            if (string.IsNullOrWhiteSpace(token))
            {
                ...
                TempData["SwalText"] = "Si el correo existe en el sistema, se enviaron instrucciones de recuperaci�n.";
                return RedirectToAction("Index");
            }

            var link = Url.Action("ActualizarPassword", "Home", new { token }, Request.Scheme);

            var asunto = "Restablecer contrase�a - Transportes Man�";
            var cuerpo = $@"
<p>Hola,</p>
...
<p>Este enlace expira en 30 minutos.</p>";
            ...
            {
                ViewBag.Mensaje = "Fall� el env�o de correo. " + ex.Message;
                return View();
            }
        ...
        public async Task<IActionResult> ActualizarPassword(string token, string new_password, string confirm_password)
        {
            ...
            {
                ViewBag.Mensaje = "Las contrase�as no coinciden.";
                ViewBag.Token = token;
                return View("ResetPassword");
            }
            ...
            if (!resp.IsSuccessStatusCode)
            {
                var msg = "No se pudo actualizar la contrase�a.";
                try
                {
                    var pd = await resp.Content.ReadFromJsonAsync<ProblemDetails>();
                    if (!string.IsNullOrWhiteSpace(pd?.Detail)) msg = pd.Detail;
            ...
            TempData["SwalType"] = "success";
            TempData["SwalTitle"] = "Listo";
            TempData["SwalText"] = "Contrase�a actualizada. Ya puede iniciar sesi�n.";
            return RedirectToAction("Index");
        }
